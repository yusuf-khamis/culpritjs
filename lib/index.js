  ...
  for (const key in schema) {
    if (Object.prototype.hasOwnProperty.call(schema, key)) {
      ...
      const prepareMessagesObject = function (propType, propRule) {
        ...
        if (typeof customMessages[key + '.' + propRule] === 'string' && customMessages[key + '.' + propRule].length > 0) {
          ruleErrorMessages.property = customMessages[key + '.' + propRule]
        }

        if (typeof customMessages[key + '.' + propRule + '.' + 'property'] === 'string' && customMessages[key + '.' + propRule + '.' + 'property'].length > 0) {
          ruleErrorMessages.property = customMessages[key + '.' + propRule + '.' + 'property']
        }

        if (typeof customMessages[key + '.' + propRule + '.' + 'arg'] === 'string' && customMessages[key + '.' + propRule + '.' + 'arg'].length > 0) {
      ...
      rules.slice(sliceIndex).forEach(function (rule) {
        ...
        validateField(newObject, key, propType, msgObj, errorMessages, rule, customArgs[key + '.' + rule[0]])
      })
    }
  }
