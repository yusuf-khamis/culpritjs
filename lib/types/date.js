...
function validate (property, object, rule, args, messages, rawMessage, momentDate = null) {
  switch (rule) {
    ...
    case 'equals': {
      ...
      if (convertToMoment(object[property]).isSame(getMoment(args, momentDate, rule))) {
        return true
      }
...
function validateArgs (rule, args, momentDate, value) {
  ...
  if (!args && typeof momentDate === 'string' && !momentDate.match(dateRegex)) {
    throw new Error(rule + ' sub-args should have valid date format')
  }

  if (!args && typeof momentDate !== 'string' && !(momentDate instanceof moment) && !(momentDate instanceof Date)) {
