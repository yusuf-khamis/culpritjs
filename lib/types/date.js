...
const dateRegex = /^(0?[0-9]|1[0-2])([/])(0?[0-9]|[0-2][0-9]|3[0-1])([/])(\d{4})$/

function validate (property, object, rule, args, messages, msgObj, momentDate = null) {
  switch (rule) {
    case 'after': {
      if (!args && !momentDate) {
      ...
      const argMoment = getMoment(args, momentDate)

      if (convertToMoment(object[property]).isAfter(argMoment)) {
        return true
      }

      messages.push(msgObj.property.replace(':field', property).replace(':value', argMoment.toString()))
    ...
    case 'after_or_equal': {
      ...
      if (convertToMoment(object[property]).isSameOrAfter(argMoment)) {
        return true
      }
    ...
    case 'before': {
      ...
      if (convertToMoment(object[property]).isBefore(argMoment)) {
        return true
      }
    ...
    case 'before_or_equal': {
      ...
      if (convertToMoment(object[property]).isSameOrBefore(argMoment)) {
        return true
      }
    ...
    case 'equals': {
      ...
      if (convertToMoment(object[property]).diff(argMoment, 'days') === 0) {
        return true
      }
...
function getMoment (arg, momentDate) {
  let dateArg
